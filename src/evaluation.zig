...
fn pestoEval(board: *const Board) i32 {
    ...
    const game_phase_table = [_]i32{ 0, 1, 1, 2, 4, 0 };
    for (0..NUM_PIECE) |pcx_idx| {
    ...
    const mid_score = mid_white - mid_black;
    const end_score = end_white - end_black;
    if (game_phase > 24) game_phase = 24;
    var score: i32 = mid_score *% game_phase +% end_score *% (24 - game_phase);
    score = @divFloor(score, 24);
    return if (board.state.turn == Side.White) score else -score;
}

fn tableIndexFlipped(idx: usize) usize {
    const r = idx / 8;
    const f = idx % 8;
    return (7 - r) * 8 + f;
}
